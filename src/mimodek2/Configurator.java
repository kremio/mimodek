...
public class Configurator {
	...
	public static void createConfigurator(PApplet app) {
		...
		Configurator.setSetting("LIGHT_MAX_RADIUS", 24f, "Controls the size of the halo.");
		
		/*
		 * Food settings
		 */
		Configurator.setSetting("FOOD_SIZE",10f,"Size of a piece of food.");
	...
	/*
	 * Return the list of all settings name and description in a string, one name per line
	 */
	public static String list(){
		StringBuilder keysList = new StringBuilder();
		Enumeration<String> keys = config.settings.keys();
		String key;
		while(keys.hasMoreElements()){
			key = keys.nextElement();
			keysList.append(key+"\t"+config.settingsDescription.get(key)+"\n");
		}
	...
	public static String help() throws ClassNotFoundException{
		StringBuilder staticMethodsList = new StringBuilder();
		String params;
		for( Method method :  ((Class<Configurator>) Class.forName("mimodek2.Configurator")).getMethods()  ){
			...
			if( method.getParameterCount() > 0 ){
				for( Parameter parameter : method.getParameters() ){
					//NOTE: this returns something like argN if the class is not compiled with the -parameters flag
					params += ", " + parameter.getName(); 
				}
				params = params.substring(2);
			}
			staticMethodsList.append( method.getName()+'('+params+')'+"\n" );
		}
		
		return staticMethodsList.toString();
	}
	...
	protected void setSettng(String name, Object value) {
		Object currentVal = settings.get(name);
		if( currentVal == null ){
	...
	public static void loadFromFile(String fileName) {
		...
		Verbose.debug("Settings file loaded successfully.");
	}
